    ...
    public class DAONguoiDung : ConnectorFactory
    {
        ...
        public int update(NGUOIDUNG ett)
        {
            String sql = String.Format("UPDATE NGUOIDUNG " +
                "SET tenNguoiDung = N'{0}',gioiTinh = N'{1}',sDT = '{2}',email = '{3}',taiKhoan = '{4}',matKhau = '{5}' WHERE maNguoiDung={6}"
                , ett.tenNguoiDung, ett.gioiTinh, ett.sDT, ett.email, ett.taiKhoan, ett.matKhau,ett.maNguoiDung);
